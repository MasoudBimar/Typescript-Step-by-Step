...
- [TypeScript Advanced Types](#typescript-advanced-types)
  ...
  - [Typescript type heirarchy](#typescript-type-heirarchy)
  - [Type Space vs Value Space](#type-space-vs-value-space)
    - [Key Differences: Type Space vs Value Space](#key-differences-type-space-vs-value-space)
      - [1. `this` keyword](#1-this-keyword)
      - [2. `&` and `|` operators](#2--and--operators)
      - [3. `const` keyword](#3-const-keyword)
      - [4. `extends` keyword](#4-extends-keyword)
...
```ts
type User = { name: string }; // User is a type in the type space
const User = { name: "Alice" }; // User is a value in the value space
```
...
// js code generated by the above class and enum
...
### Key Differences: Type Space vs Value Space

#### 1. `this` keyword

**Value Space:** JavaScript's `this` keyword (runtime context)

```ts
const obj = {
  name: "Alice",
  greet() {
    console.log(this.name); // 'this' refers to obj at runtime
  },
};
obj.greet(); // Output: "Alice"
```

**Type Space:** `this` as a polymorphic type for method chaining
...
const result = new AdvancedBuilder().add("hello").shout(); // Returns AdvancedBuilder instance, not Builder
```

#### 2. `&` and `|` operators

**Value Space:** Bitwise AND and OR operations

```ts
const a = 5; // 0101 in binary
const b = 3; // 0011 in binary

const and = a & b; // 0101 & 0011 = 0001 (1)
const or = a | b; // 0101 | 0011 = 0111 (7)

console.log(and); // 1
console.log(or); // 7
```

**Type Space:** Union and intersection operators

```ts
// Union type (|)
type Status = "pending" | "completed" | "failed";
let status: Status = "pending"; // One of three values

// Intersection type (&)
type Name = { firstName: string };
type Age = { age: number };
type Person = Name & Age; // Must have both properties
...
**Value Space:** Declares a read-only variable
...
**Type Space:** `as const` changes type inference to literal types
...
**Value Space:** Defines class inheritance
...
**Type Space:** Defines subtype relationships and generic constraints
...
**Value Space:** Checks object properties and iterates over them
...
**Type Space:** Used in mapped types to create new types

```ts
// Create a type with readonly versions of all properties
type ReadonlyUser = {
...
// Map properties to a specific type
type Getters<T> = {
...
**Value Space:** Logical NOT operator
...
**Type Space:** Non-null assertion operator
...
// const length = value.length; // Error: value might be null

// Use ! to assert it's not null
const length = value!.length; // OK, length = 5

// Common in DOM operations
const element = document.getElementById("myId");
const width = element!.clientWidth; // Assert element exists
```
